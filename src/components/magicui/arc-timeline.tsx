...
import { ComponentPropsWithoutRef, ReactNode, useState, useRef, useCallback, useEffect } from "react";
...
export function ArcTimeline(props: ArcTimelineProps) {
  ...
  // Calculate initial position based on defaultActiveStep
  ...
  const [circleContainerRotateDeg, setCircleContainerRotateDeg] = useState(getInitialPosition);

  // Calculate which step is closest to the top position
  const getClosestStep = useCallback(() => {
    let closestStep = { lineIndex: 0, stepIndex: 0, distance: Infinity };
    ...
    data.forEach((line, lineIndex) => {
      line.steps.forEach((step, stepIndex) => {
        ...
        const totalAngle = angle + circleContainerRotateDeg;
        ...
        if (distance < closestStep.distance) {
          closestStep = { lineIndex, stepIndex, distance };
        }
      });
    });
    
    return closestStep;
  }, [data, circleContainerRotateDeg, angleBetweenMinorSteps, lineCountFillBetweenSteps, boundaryPlaceholderLinesCount]);
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    e.preventDefault();
  }, [getAngleFromCenter, circleContainerRotateDeg]);
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    const angleDifference = currentAngle - dragStartAngle;
    const newRotation = dragStartRotation + angleDifference;
    
    // Limit rotation to prevent over-rotation
    const maxRotation = 360 * 3; // Allow 3 full rotations
    const clampedRotation = Math.max(-maxRotation, Math.min(maxRotation, newRotation));
    
    setCircleContainerRotateDeg(clampedRotation);
  }, [isDragging, getAngleFromCenter, dragStartAngle, dragStartRotation]);
  ...
  return (
    <div
      {...restProps}
      ref={timelineRef}
      className={cn("relative h-[500px] w-full overflow-visible", className)}
    >
      <div
        ref={circleRef}
        style={{
        ...
        className={cn(
          "absolute left-1/2 top-28 aspect-square origin-center rounded-full transition-all duration-500 ease-in-out",
          ...
          return (
            <div key={`${lineIndex}`}>
              {line.steps.map((step, stepIndex) => {
                ...
                // check if the step is active - only the closest step to the top position
                const isActive = lineIndex === closestStep.lineIndex && stepIndex === closestStep.stepIndex;
                return (
                    ...
                    >
                      ...
                      >
                        ...
                        {step.image && (
                          <div
                            className={cn(
                              "absolute bottom-8 left-1/2 -translate-x-1/2 translate-y-[calc(100%_+_120px)] transition-opacity duration-300 ease-in",
                              isActive ? "opacity-100" : "opacity-0",
                            )}
                            style={{
                              width: "256px",
